...
routes:
  /api/auth/: http://auth:8000/api/auth/  # Просто укажи свой URL, и он летит!
plugins:
...
groups:
  - config:
      routeLenHint: 256  # Чтобы твои запросы летели как ракеты
    routes:
      /api/auth/get_user: GET http://auth:8000/api/auth/get_user
  - plugins:
    - kind: logger
      args:
        level: debug  # Убираем секреты в тени, но логи — на высшем уровне
    routes:
      /api/notifier/: http://notifier:8000/api/
```
